...
### 1.2 데스크탑(admin 또는 관리자) 화면 기준

| 항목 | 기준 |
| :---- | :---- |
| 화면 폭 | **1400px** 고정 (화면설계서·프로토타입 프레임 동일) |
| 레이아웃 | GNB 56px(상단) + LNB 220px(좌측) + 콘텐츠 1180px |
| 콘텐츠 패딩 | 상 24px / 우 32px / 하 40px / 좌 40px |
| 컨트롤 | 입력 필드·버튼 높이 36px, radius 6px, 본문 13px (소형 버튼 32px) |
| 목록 | 데이터 그리드(헤더 40px·행 44px), 컬럼 18종은 그리드 내부 가로 스크롤 |
| 입력 폼 | 라벨(140px) + 값 폼 테이블, 한 행 2쌍 배치 |
| 정책 전환 | 모바일 기준 작성규칙(프레임 393px·탭바·바텀시트·숫자 키패드 등)은 데스크탑 정책으로 전환 — 상세는 화면설계서 「데스크탑 정책 전환 기준」 표 참조 |
---
