...
:root {
  ...
  --fth-w: 140px;        /* 폼 테이블 레이블 셀(.fth) 폭 (§8-4, v5.2.2) */
  --ftd-min: 320px;      /* 폼 테이블 값 셀(.ftd) 최소 폭 — 1그룹 최소폭 = fth + ftd-min = 460 (§8-4, v5.2.2) */
}
```
...
.figma-screen-wrap [data-figma-layer-name="screen/{화면ID}"]
  ...
  │                       └── .fbox > .frow > .fth + .ftd (+ .fth + .ftd)   (폼 테이블 — v5.2.2, 아래 「폼 테이블」)
...
| `.fbox` (v5.2.2) | 폼 테이블 컨테이너 / `container-type: inline-size` (1그룹 전환 기준 = 자기 폭) / 상단 테두리 1px solid #C9D4E4 / 폭 부모 전체 |
| `.frow` · `.frow.two` (v5.2.2) | **기본값 two** — 클래스 없이 `frow`만 써도 two로 동작 / `display: grid` / 열 `var(--fth-w) minmax(0,1fr) var(--fth-w) minmax(0,1fr)` / 하단 테두리 1px solid #C9D4E4 |
| `.frow.one` (v5.2.2) | 항상 1그룹 고정이 필요한 행에만 명시 / 열 `var(--fth-w) minmax(0,1fr)` |
| `.fth` (v5.2.2) | 레이블 셀 / 폭 `var(--fth-w)` 140 / 배경 #F5F8FC / 13px 600 --t2 / 패딩 10px 12px / 좌측 테두리 1px solid #C9D4E4 / `position: relative`(번호 뱃지 기준) / 필수 표시는 레이블 뒤 `*` (--danger) |
| `.ftd` (v5.2.2) | 값 셀 / 최소 폭 `var(--ftd-min)` 320 (1그룹 판단 기준) / 13px --t1 / 패딩 8px 12px / 좌·우 테두리 1px solid #C9D4E4 / `min-width: 0` / `position: relative` / 안의 폼 요소는 §4-1 실제 태그 |

**폼 테이블 (`.fbox` › `.frow` › `.fth`·`.ftd`) — v5.2.2 신설**

> Admin 화면의 검색 조건·등록·상세 영역처럼 「레이블 | 값」이 반복되는 표 형태 폼은 `<table>`이 아닌 `.fbox` 그리드로 그린다. **`.frow` 기본값은 `two`**(한 행에 fth·ftd 2그룹)이고, 가로 폭이 모자라면 **자동으로 1그룹**으로 바뀐다.

| 규칙 | 내용 |
|---|---|
| 기본값 | `.frow`는 기본 `two`. 클래스를 생략해도 `frow two`와 같다. 산출물에서는 `class="frow"`로 쓰고, 1그룹 고정이 필요한 행만 `frow one`을 쓴다. `three` 이상 금지 |
| 그룹 | `.fth` + `.ftd` 한 쌍 = 그룹 1개. `.frow` 자식은 `.fth`·`.ftd`만, 이 순서로 교대. two는 1~2그룹, one은 1그룹 |
| 1그룹 자동 전환 | 기준은 뷰포트가 아니라 **`.fbox` 자신의 가로 폭**(container query). 2그룹 최소폭 = 2 × (`--fth-w` 140 + `--ftd-min` 320) = **920px**. `.fbox` 폭이 919px 이하이면 모든 two 행이 1그룹이 되고, 두 번째 그룹은 다음 줄로 내려간다(마크업 변경·JS 없음) |
| 그룹 1개뿐인 행 | two 행에 그룹이 1개면 `.ftd`가 남은 폭 전체를 차지한다(`grid-column: 2 / -1`). 이메일 주소·메모·첨부처럼 넓은 값은 그룹 1개로 쓰면 되고, 별도 span 클래스를 만들지 않는다 |
| 적용 결과 | Admin 본문(1280 − 좌우 패딩 64 = 1216px) → 2그룹. 팝업·레이어 팝업·좁은 카드 안 `.fbox`(920px 미만) → 1그룹 |
| 번호 뱃지 | `.fth`에 단다(§5 기본 위치). 1그룹으로 줄바뀜돼도 뱃지는 해당 셀을 따라간다. 값 영역 전체를 가리키는 경우만 `.ftd`에 단다 |
| 피그마 임포트 | 임포트 시점의 계산된 레이아웃(2그룹 또는 1그룹)이 그대로 들어간다. Admin 기본 산출물은 1920px 뷰포트에서 임포트한다(§8-4 예산) |

```html
<div class="fbox" data-figma-layer-name="3/fbox-issue-info">
  <div class="frow">                                   <!-- 기본값 two : 2그룹 -->
    <div class="fth">신청일시</div><div class="ftd">2025-05-05 12:30:25</div>
    <div class="fth">발급일시</div><div class="ftd">-</div>
  </div>
  <div class="frow">                                   <!-- 그룹 1개 → ftd가 남은 폭 전체 -->
    <div class="fth"><label for="issueMemo">발급 메모</label></div>
    <div class="ftd"><textarea id="issueMemo" placeholder="발급 메모 입력"></textarea></div>
  </div>
  <div class="frow one">…</div>                        <!-- 1그룹 고정이 필요할 때만 -->
</div>
```

```css
.fbox { container-type: inline-size; border-top: 1px solid #C9D4E4; }
.frow, .frow.two {                                                 /* 기본값 two */
  display: grid;
  grid-template-columns: var(--fth-w) minmax(0,1fr) var(--fth-w) minmax(0,1fr);
  border-bottom: 1px solid #C9D4E4;
}
.frow.one { grid-template-columns: var(--fth-w) minmax(0,1fr); }
.frow > .ftd:nth-child(2):last-child { grid-column: 2 / -1; }      /* 그룹 1개뿐인 행 */
@container (max-width: 919px) {                                    /* 2 × (140 + 320) = 920 미만 → 1그룹 */
  .frow, .frow.two { grid-template-columns: var(--fth-w) minmax(0,1fr); }
  .frow > .fth:nth-child(3), .frow > .ftd:nth-child(4) { border-top: 1px solid #C9D4E4; }
}
.fth { background: #F5F8FC; font: 600 13px/1.5 inherit; color: var(--t2); padding: 10px 12px;
       border-left: 1px solid #C9D4E4; position: relative; }
.ftd { font-size: 13px; color: var(--t1); padding: 8px 12px; min-width: 0;
       border-left: 1px solid #C9D4E4; border-right: 1px solid #C9D4E4; position: relative; }
```
- `--fth-w`·`--ftd-min`을 바꾸면 전환 기준(920px)도 함께 다시 계산해 `@container` 값을 고친다(CSS 변수는 `@container` 조건에 쓸 수 없음).
- 목록형 데이터(검색 결과 표 등)는 폼 테이블이 아니므로 `<table>`을 그대로 쓴다. `.fbox`는 레이블·값 폼에만 쓴다.
...
- 레이블·값 폼 테이블을 `<table>`·`colspan` 또는 행마다 인라인 `grid-template-columns`로 작성하는 것 금지, `frow three` 이상 금지 (v5.2.2) → §8-4 「폼 테이블」
...
## 14. 산출물 검증 절차 (v3 신설 · v5·v5.1·v5.2·v5.2.2 항목 추가)
...
| 12 | 폼 테이블 (v5.2.2) | `.frow` class 값이 `frow` · `frow two` · `frow one` 외 0건 / 각 `.frow` 자식이 `.fth`·`.ftd` 교대(첫 자식 `.fth`), 그룹 수 two 1~2 · one 1 / CSS에 `.fbox { container-type: inline-size }` · 기본 `.frow` 4열 `grid-template-columns` · `@container (max-width: 919px)` 1그룹 규칙 · `.ftd:nth-child(2):last-child` 전체 폭 규칙 존재 / `--fth-w`·`--ftd-min` 토큰 존재, `@container` 값 == 2 × (fth + ftd-min) − 1 / `.fbox` 안 `<table>`·`colspan`·인라인 `grid-template-columns` 0건 / 육안: 1920px에서 Admin 본문 `.fbox` 2그룹, 920px 미만 컨테이너(팝업 등)의 `.fbox` 1그룹 전환·줄바뀐 그룹 구분선·뱃지 위치 확인 |
