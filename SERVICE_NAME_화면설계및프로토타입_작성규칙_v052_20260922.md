...
:root {
  ...
  --adm-frame-w: 1280px; /* Admin 프레임 폭 — LNB 숨김(기본) 상태 (§8-4) */
  --adm-lnb-w: 200px;    /* Admin 프레임 좌측 LNB 폭 — body.adm-lnb-visible 일 때만 (§8-4) */
  --anno-w: 320px;       /* description 패널 폭 — Admin 화면 기준 (§8-4) */
  --canvas-gap: 16px;    /* 프레임 ↔ description 패널 간격 (§8-4) */
}
```
...
### 5-1-1. 클리핑(잘림) 방지 (v5.2 신설)
뱃지가 절반만 보이거나 안 보이는 현상의 대부분은 **가림이 아니라 잘림**이다. `z-index`는 조상의 `overflow` 밖으로 요소를 꺼내 주지 못하므로 아래 규칙으로 구조에서 막는다.

| 규칙 | 내용 |
|---|---|
| 1. 클리핑 컨테이너 안에 뱃지 금지 | `overflow: hidden / auto / clip`인 요소(팝업 `.sysdlg`·바텀시트·카드·이미지 래퍼·`.scroll-body` 자체)의 **직계 자식으로 `.bdg`를 두지 않는다.** 뱃지는 그 요소를 감싼 바깥 `.rel` 래퍼(`position: relative`, overflow 없음)에 붙인다 |
| 2. 둥근 모서리에 `overflow: hidden` 쓰지 않기 | 배경·테두리는 `border-radius`만으로 둥글어진다. 내부 스크롤이 필요하면 스크롤 영역(`.dlg-body`·`.sheet-body`)에만 `overflow: auto`를 주고 컨테이너 자체는 열어 둔다 |
| 3. 프레임 모서리에 붙는 요소 | `.phone-screen` 좌측 끝에 딱 붙는 풀블리드 헤더·배너·섹션 타이틀은 뱃지가 `left:-10px`로 걸칠 공간이 없어 프레임에 잘린다. **`.bdg.in`(요소 안쪽 좌상단, `top:6px; left:6px`)을 쓰거나**, 해당 블록에 §7과 같은 `margin-left: 32px`을 주어 걸칠 공간을 만든다 |
| 4. stacking context 만드는 조상 주의 | `transform`·`opacity < 1`·`filter`·`will-change`·`contain`이 걸린 조상 안에서는 `.bdg`의 `z-index:100`이 바깥 요소와 비교되지 않는다(진짜 가림). 슬라이드 애니메이션은 `transform` 대신 `top/bottom` 전환을 쓰거나, 뱃지를 그 조상 밖 `.rel`에 둔다 |
| 5. 진단 절차 | 개발자도구에서 잘린 `.bdg`를 선택한 뒤 조상을 올라가며 Computed `overflow`가 `hidden/auto/clip`인 첫 요소를 찾는다(범인). 그 요소에 `transform` 등이 있으면 규칙 4 케이스 |

- `.bdg.in` 스타일: `top:6px; left:6px;` 나머지는 `.bdg`와 동일. 안쪽 배치라 요소의 첫 텍스트를 덮을 수 있으므로 해당 요소에 `padding-left: 28px`를 함께 준다.
- 위 규칙을 지키기 어려운 구조(중첩 팝업·가로 스크롤 캐러셀 등)는 §5-2 뱃지 레이어 방식으로 전환한다.

### 5-2. 뱃지 레이어 분리 (v5.2 신설 · 권장)
뱃지를 대상 요소 안에 넣지 않고, **`.phone-screen` 직속 `.badge-layer`에 모아 스크립트로 배치**한다. 어떤 `overflow`·stacking context에도 영향받지 않아 §5-1-1의 문제가 구조적으로 사라진다. 검진예약 v0.8 화면설계서(`BADGE_MAP` + JS 배치)가 이 방식이다.

**구조**
```html
<div class="phone-screen" id="screen-HospitalList">
  <div class="scroll-body">
    <div class="hcard HospitalList-list-1">…</div>       <!-- 뱃지 없음, Component ID class만 -->
  </div>
  <div class="badge-layer"></div>                          <!-- 뱃지가 여기에 생성됨 -->
</div>
<script>
  // 화면별 뱃지 맵: 번호 → 대상 selector (+ 위치 변형)
  const BADGE_MAP = {
    HospitalList: [
      { n: '1', sel: '.HospitalList-list-1' },
      { n: '2', sel: '.HospitalList-modal-1', pos: 'out' },
      { n: '3', sel: '.HospitalList-tab-2', pos: 'tab-end' }
    ]
  };
</script>
```
...
**스타일**
```css
.badge-layer { position: absolute; inset: 0; pointer-events: none; z-index: 100; overflow: visible; }
.badge-layer .bdg { position: absolute; transform: translate(-10px, -10px); }   /* 기본: 대상 좌상단 -10/-10 */
.badge-layer .bdg.in      { transform: translate(6px, 6px); }
.badge-layer .bdg.out     { transform: translate(-28px, -1px); }
.badge-layer .bdg.tab-end { transform: translate(-10px, -50%); }
.badge-layer .bdg.bar     { transform: translate(-100%, -50%); }   /* left = 대상 우측 - 8px */
```

**배치 스크립트** (§8-2 사이드바 스크립트와 같은 `<script>`에 둔다)
```javascript
function placeBadges(key) {
  const screen = document.getElementById('screen-' + key); if (!screen) return;
  const layer = screen.querySelector('.badge-layer'); if (!layer) return;
  layer.innerHTML = '';
  const base = screen.getBoundingClientRect();
  (BADGE_MAP[key] || []).forEach(b => {
    const t = screen.querySelector(b.sel); if (!t) return;
    const r = t.getBoundingClientRect();
    const el = document.createElement('span');
    el.className = 'bdg' + (b.pos ? ' ' + b.pos : '') + (b.tbd ? ' tbd' : '');
    el.textContent = b.n;
    const top = r.top - base.top + (b.pos === 'tab-end' || b.pos === 'bar' ? r.height / 2 : 0);
    const left = b.pos === 'bar' ? r.right - base.left - 8 : r.left - base.left;
    el.style.top = top + 'px'; el.style.left = left + 'px';
    layer.appendChild(el);
  });
}
document.querySelectorAll('.phone-screen[id^="screen-"]').forEach(sc => {
  const key = sc.id.replace('screen-', '');
  placeBadges(key);
  sc.querySelectorAll('.scroll-body').forEach(sb => sb.addEventListener('scroll', () => placeBadges(key)));
});
window.addEventListener('resize', () => document.querySelectorAll('.phone-screen[id^="screen-"]').forEach(sc => placeBadges(sc.id.replace('screen-', ''))));
```

**규칙**
- `.badge-layer`는 `.phone-screen`(또는 `.admin-frame`, §8-4)의 **마지막 자식**으로 두고, `.phone-screen`은 `position: relative`를 유지한다.
- 대상 요소는 `BADGE_MAP`의 `sel`로만 지정한다(Component ID class 사용, §4). 마크업에 `.bdg`를 직접 쓰지 않는다 — 두 방식을 한 문서에 섞지 않는다.
- 위치 변형은 §5-1 표와 동일(`in` · `out` · `tab-end` · `bar`), 미확정은 `tbd: true`.
- `.scroll-body`가 스크롤되거나 창 크기가 바뀌면 재배치한다(위 스크립트). 뷰포트 밖으로 스크롤된 대상의 뱃지는 `.badge-layer`의 `overflow: visible` 때문에 프레임 밖에 그려질 수 있으므로, `r.top < base.top || r.bottom > base.bottom`이면 건너뛴다(선택).
- 피그마 임포트: 레이어의 뱃지도 DOM에 존재하므로 그대로 임포트된다. 임포트 전 `placeBadges`가 한 번 실행된 상태인지 확인한다.
- §14 항목 2(넘버링 매핑)는 이 방식에서 `BADGE_MAP[key].n` 집합 == description `.an` 번호 집합으로 검사한다.
 ...
 └── .figma-canvas [data-figma-layer-name="{SERVICE_NAME}/화면설계"]
       └── .vpair × N                             ← v5 신설: 그룹 박스 (사이드바 .pg-grp와 1:1)
             ...
             └── .figma-screen-wrap × N [data-figma-layer-name="screen/{화면ID}"]
                   ...
                   │     └── .phone-frame [data-figma-layer-name="frame/{화면명}"]   (모바일)  |  .admin-frame (= .phone-screen.adm, Admin, §8-4)
                   │           ├── .status-bar
...
| 동작 | 규칙 |
|------|------|
| 항목 클릭 → 이동 (v5.2 변경) | 기준 요소 = `#screen-{key}`가 속한 `.figma-screen-col` 안의 **`.ptitle h1`**(없으면 `.figma-label`, 그것도 없으면 컬럼). 그 요소의 **좌측 X = 사이드바 폭(접힘 시 34px) + 24px, 상단 Y = 24px** 위치로 `window.scrollTo({behavior:'smooth'})`. 이동 후 그 `h1`에 `tabindex="-1"`을 주고 `focus({preventScroll:true})`로 포커스를 옮긴다(키보드 탐색 시작점·스크린리더 기준점) |
| 이동 결과 보장 (v5.2) | 이동 후 **화면 목록(사이드바) · 화면 프레임 · description 패널(`.annotation-panel`)이 모두 뷰포트 안에** 있어야 한다. `.figma-screen-wrap`(프레임+패널)의 우측 끝이 `innerWidth - 16`을 넘으면 그만큼 X를 더 이동하되, 그 결과 프레임 좌측이 사이드바 밑으로 들어가면 **패널을 우선**하고 사이드바 접기를 안내 토스트로 권한다. 1920px 뷰포트·기본 사이드바에서는 §8-4 예산으로 항상 충족되므로 보정이 발생하지 않는다 |
...
(function () {
  const go = key => {
    ...
    const r = title.getBoundingClientRect(), w = wrap.getBoundingClientRect();
    let left = window.scrollX + r.left - pagesW - 24;                                    // 제목을 사이드바 우측 24px에
    const need = window.scrollX + w.right + 16 - window.innerWidth;                       // 패널 우측 끝이 보이려면 필요한 scrollX
    if (need > left) left = need;                                                         // 패널이 잘리면 패널 우선 (§8-2 이동 결과 보장)
    window.scrollTo({ left, top: window.scrollY + r.top - 24, behavior: 'smooth' });
  ...
  if (pg && rz) {
    ...
    const DEF = 264;
    const setW = w => document.documentElement.style.setProperty('--pages-w', Math.min(MAX, Math.max(MIN, Math.round(w))) + 'px');
  ...
  // v5.2 — §8-4 Admin LNB 표시 토글 (기본 숨김, 저장하지 않음)
  const lt = document.getElementById('lnbToggle');
  if (lt) lt.addEventListener('click', () => lt.classList.toggle('on', document.body.classList.toggle('adm-lnb-visible')));
})();
```
...
.figma-screen-wrap [data-figma-layer-name="screen/{화면ID}"]
  ...
  │     └── .admin-frame.phone-screen.adm#screen-{화면ID} [data-figma-layer-name="frame/{화면명}"]   폭 var(--adm-frame-w) 1280px
  │           ├── .adm-gnb                  (상단 바 56px — 로고 · 관리자명 · 로그아웃)
  │           └── .adm-body                 (flex row)
  │                 ├── .adm-lnb            (좌측 메뉴 200px — **기본 숨김**, body.adm-lnb-visible 일 때만 표시)
  │                 └── .admin-main         (본문 — 프레임 폭 전체 1280px · 패딩 24px 32px)
  └── .annotation-panel                     (우측 description, 폭 var(--anno-w) 320px)
```

**`.adm-lnb` 기본 숨김**
- 좌측 화면 목록 사이드바(§8-2)가 메뉴 역할을 하므로 **화면설계서에서는 `.adm-lnb`를 기본 숨김**으로 두고, 프레임 폭은 본문만으로 1280px(`--adm-frame-w`)로 잡는다. 실제 서비스 화면에 LNB가 있다는 것은 `.adm-gnb` 우측 끝의 `LNB 숨김 · 화면 목록 참조` 안내 칩(11px, --t3)으로 표시한다.
- LNB 자체를 그려야 할 때(LNB 메뉴 구성 정의 화면, 피그마 임포트에 LNB 포함)는 `.doc-head`의 `LNB 표시` 토글 버튼(`#lnbToggle`)으로 `body.adm-lnb-visible`을 켠다. 이때 프레임 폭은 1280 + 200 = 1480이 되어 1920px에서 description 패널이 뷰포트 밖으로 밀리므로, 사이드바를 접거나(§8-2) 가로 스크롤을 허용한다. 저장하지 않는다(§13).
- LNB의 번호 뱃지·description 항목은 숨김 상태에서도 DOM에 유지한다(§14 항목 2 넘버링 매핑 영향 없음). LNB 메뉴 구성은 별도 화면 `{화면ID}` 1개(예: `AdminLnb`)에서만 정의하고, 나머지 Admin 화면 description에는 LNB 항목을 반복하지 않는다.

**1920px 가로 예산 — 화면 목록 · 프레임 · description 패널이 한 화면에**
| 요소 | 폭 |
|---|---|
| `aside.pages` (화면 목록) | 264 (기본) |
| 캔버스 좌측 패딩 | 24 |
| `.admin-frame` (예: 서류신청관리) | 1280 |
| 프레임 ↔ 패널 간격 `--canvas-gap` | 16 |
| `.annotation-panel` (Description) | 320 |
| 캔버스 우측 패딩 | 16 |
| **합계** | **1920** |

- 사이드바에서 항목을 클릭해 이동하면 위 세 영역(화면 목록 · 프레임 · Description)이 **모두 뷰포트 안에** 있어야 한다(§8-2 「이동 결과 보장」). 1920px·기본 사이드바에서 정확히 맞으므로 프레임·패널·간격·패딩을 **이 값보다 키우지 않는다.**
- `.admin-main`의 내용은 1280px 안에서 완결한다. 표가 넓으면 `.admin-main` 안에서 `overflow-x: auto`로 처리하고 프레임 폭을 키우지 않는다.
- `.annotation-panel`은 320px 고정, 세로는 프레임 높이와 같게(`align-self: stretch`) 두고 내용이 넘치면 패널 내부 스크롤(`overflow-y: auto`, max-height = 프레임 높이). description 항목이 많아도 프레임 아래로 늘어나게 두지 않는다.
- 사이드바를 264px보다 넓히면(§8-2 폭 조절) 그만큼 패널이 밀리므로, 이동 스크립트가 패널 우선으로 X를 보정한다. 1920px 미만 뷰포트에서 세 영역을 모두 보려면 사이드바를 접는다(34px → 여유 230px).

**뷰포트별 처리**
| 뷰포트 | 처리 |
|---|---|
| ≥ 1920 · 사이드바 264 | 보정 없음. 세 영역 모두 표시 |
| 1690~1919 | 사이드바 접힘(34px)이면 세 영역 표시. 펼친 상태면 이동 시 패널 우선 보정 + 「사이드바를 접으면 전체가 보입니다」 토스트(3초) |
| < 1690 | 패널 우선 보정. 프레임 좌측 일부가 사이드바 밑으로 들어갈 수 있음 |
| `body.adm-lnb-visible` | 프레임 1480 → 어떤 뷰포트에서도 1920 예산 초과. 가로 스크롤 허용, 피그마 임포트 전용 상태로만 사용 |

- **피그마 임포트**: LNB 없는 프레임(1280)이 기본 산출물이다. LNB 포함 프레임이 필요하면 `body.adm-lnb-visible`을 켜고 ≥ 2120px 뷰포트에서 임포트한다(§14 항목 11).

**규격**
| 요소 | 규격 |
|---|---|
| `.admin-frame` (`.phone-screen.adm`) | 폭 `var(--adm-frame-w)` 1280 (LNB 표시 시 1480) / 최소 높이 760 / 배경 #FFFFFF / 테두리 1px solid --border / 라운드 12px / overflow hidden |
| `.adm-gnb` | 높이 56 / 배경 --pd / 글자 #FFFFFF 14px 600 / 패딩 0 24px / 우측 끝 안내 칩 `LNB 숨김 · 화면 목록 참조` (11px, rgba(255,255,255,.7)) |
| `.adm-lnb` | **기본 `display: none`** — `body.adm-lnb-visible .adm-lnb { display: block }` / 폭 200 / 배경 #F5F8FC / 우측 테두리 1px solid --border / 메뉴 항목 높이 44 · 14px --t2 · 활성 --pl 배경 + --p 글자 + 좌측 3px --p |
| `.admin-main` | 폭 프레임 전체(flex 1) / 패딩 24px 32px / `.ptitle h1`과 별개로 본문 안 제목은 20px/700 |
| `#lnbToggle` | `.doc-head` 우측 상단 버튼 `LNB 표시` — §4 `.pages-toggle`과 같은 스타일(테두리 1px #D5DDE8 · 라운드 6px · 12px) / 켜짐 시 배경 --pl 글자 --pd |
| `.ptitle h1` | 20px/700 --t1 / margin 0 0 8px / `outline: none`, 포커스 시 `box-shadow: 0 0 0 2px var(--pl)` |
| `.annotation-panel` | 폭 `var(--anno-w)` 320 / 패딩 16px / 배경 #FFFFFF / 테두리 1px solid --border / 라운드 12px / `overflow-y: auto` / 프레임과의 간격 `var(--canvas-gap)` 16 |
...
- `overflow: hidden/auto/clip` 요소의 직계 자식으로 `.bdg` 배치 금지, 둥근 모서리 목적의 `overflow: hidden` 금지 (v5.2) → §5-1-1. 마크업 직접 `.bdg`와 §5-2 뱃지 레이어를 한 문서에 혼용 금지
...
| 5 | 뱃지 가림·잘림 | §5-1 기준 육안 확인 (탭·제목 좌측 배치 적용 여부) + §5-1-1 스크립트 검사: 각 `.bdg`의 조상 중 `overflow`가 `hidden/auto/clip`인 요소가 `.phone-screen`(또는 `.admin-frame`) 이전에 있으면 실패. `.bdg`의 `getBoundingClientRect()`가 `.phone-screen` 경계 안에 완전히 들어오는지 확인 |
...
| 11 | 제목·Admin 프레임 (v5.2) | 모든 `.figma-screen-col`에 `.ptitle h1` 존재 · 텍스트 == `.figma-sublabel` 화면명 / `.admin-frame` 폭 1280 · `.annotation-panel` 320 · 간격 16 · 캔버스 패딩 24/16 (합 1920 초과 금지) / `.adm-lnb` 기본 `display:none` + `body.adm-lnb-visible` 규칙 · `#lnbToggle` 존재 / 이동 스크립트에 패널 우측 보정(`need > left`) 존재 / 1920px 뷰포트에서 임의 항목 클릭 후 화면 목록·프레임·Description 세 영역 동시 표시 확인(육안) |
| 12 | 뱃지 레이어 (v5.2, §5-2 방식일 때) | 모든 `.phone-screen`에 `.badge-layer`가 마지막 자식으로 존재 / 마크업 안 `.bdg` 0건(레이어 생성분 제외) / `BADGE_MAP[key]`의 `sel`이 모두 해당 화면 안에서 1개씩 매칭 / `placeBadges` 정의·호출 존재 |
